...
void UCombatCompoment::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(UCombatCompoment, SecondaryWeapon);
...
void UCombatCompoment::EquipPrimaryWeapon(AWeapon* WeaponToEquip)
{
...
void UCombatCompoment::EquipSecondaryWeapon(AWeapon* WeaponToEquip)
{
	if(WeaponToEquip == nullptr)
		return;
...
void UCombatCompoment::SwapWeapons()
{
	if(CombatState != ECombatType::ECS_Unoccupied) return;
...
void UCombatCompoment::AttachActorToBackpack(AActor* ActorToAttach)
{
...
void UCombatCompoment::OnRep_CombatState()
{
	...
	case ECombatType::ECS_Reloading:
		if(Character && !Character->IsLocallyReloading()) HandleReload();
		break;
...
void UCombatCompoment::OnRep_SecondaryWeapon()
{
...
void UCombatCompoment::LocalFire(const FVector_NetQuantize& TraceHitTarget)
{
	if(EquippedWeapon == nullptr)
		return;
